...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	if (!isCode) {
		w = TEXTW(text) - lrpad;
		drw_text(drw, x - 2 * sp - borderpx, 0, w, bh, 0, text, 0);
	}
...
drawbar(Monitor *m)
{
	...
	if (m->num == statusmon || statusmon < 0) { /* status is only drawn on one monitor, configured by statusmon variable (draw on all monitors if statusmon is -1) */
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		// TODO: commenting this "if" removes the small squares the tags, not sure if I want with or without those...
		if (occ & 1 << i)
...
updatestatus(void)
{
	...
	for(m = mons; m; m = m->next)
	{
		if(m == selmon || m->num == statusmon || statusmon < 0)
		{
			drawbar(m);
		}
	}
